...
parser.add_argument('tc_no_range', type=int, nargs='?', default=1, help='Range of tc number (default: 1)')
...
tc_no_range = args.tc_no_range - 1
...
# validating the tc number
if tc_no_from <= 0:
  print('\x1b[31mPlease provide valid start tc numbers as arguments. \x1b[33mExample - \x1b[32mpython script.py https://no2udaipur.kvs.ac.in/ \x1b[4m3045701\x1b[0m \x1b[32m5\x1b[0m')
  sys.exit(1) # exit the program with an error code

if tc_no_range > 99:
  print('\x1b[31mRange Error: The maximum range should be 100, you have provided '+ str(tc_no_range + 1) +'\x1b[0m')
  sys.exit(1)
...
async def process_tc(tc_no, url):
    async with aiohttp.ClientSession(headers=headers) as session:
        ...
        async with session.post('https://' + url.netloc + '/views/ajax', data=payload) as response:
            html_data = json.loads((await response.text()))[2]['data']
            soup = BeautifulSoup(html_data, 'html.parser')
            table = soup.find('table')
            if table:
                print('\x1b[32m\u2713 ' + str(tc_no) + '\x1b[0m')
                if log_tc_data: process_data(table)
            else:
                print('\x1b[31m\u2717 ' + str(tc_no) + '\x1b[0m')
                tc_numbers.append(tc_no)

async def main():
    tasks = []
    for tc_no in range(tc_no_from, tc_no_from + tc_no_range + 1):
        tasks.append(process_tc(tc_no, url))
