...
async function processTCData(tcNo, html_data, logTcData) {
  const dom = new JSDOM(html_data);
  const table = dom.window.document.querySelector("table");

  if (table) {
    let tcIssuedData = `\x1b[32m\u2713 ${tcNo}\x1b[0m`;
    ...
    console.log(`\x1b[31m\u2715 ${tcNo}\x1b[0m`);
    tcNumbers.push(tcNo);
  }
}
...
async function run() {
  ...
  try {
    ...
    for (let i = 0; i < responses.length; i++) {
      ...
      await processTCData(from + i, html_data, logTcData);
    }
    ...
    if (tcNumbers.length) {
      ...
      await fs.writeFile('tc-numbers.txt', tcNumbers.join('\n'));
      console.log('Saved!');
    }
  } catch (error) {
